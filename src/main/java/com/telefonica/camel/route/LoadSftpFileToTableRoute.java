...
public class LoadSftpFileToTableRoute extends EndpointRouteBuilder{
	...
	from(getSftpOriginServer(transferFilesProperties))
			.log("init 1" + transferFilesProperties.getProcessName())
		.routeId(transferFilesProperties.getProcessName())
		.autoStartup(false)
		.setProperty(KeyProperty.UUID).method(Util.class, "getUUID")
		.log(LoggingLevel.INFO, logTransfer, Util.log("start process download file ScoreRC_${date:now:ddMMyy}.txt"))
			.log(LoggingLevel.INFO, logTransfer, Util.log("Body: ${body}"))
		.choice()
			.when(body().isNotNull())
			    .log(LoggingLevel.INFO, logTransfer, Util.log("Starting file transfer..."))
				.to(toScpDestinationServer(transferFilesProperties))
				.log(LoggingLevel.INFO, logTransfer, Util.log("File transfer completed."))
				.log(LoggingLevel.INFO, logTransfer, Util.log("end process download file ScoreRC_${date:now:ddMMyy}.txt"))
			.otherwise()
				.log(LoggingLevel.WARN, logTransfer, Util.log("not found file ScoreRC_${date:now:ddMMyy}.txt"))
				.log(LoggingLevel.INFO, logTransfer, Util.log("end process"))
		.endChoice()
		.end()
		.filter(constant(transferFilesProperties.getExecutionType()).isEqualTo(ExecutionType.MANUAL))
			.log("error")
			.to(controlbus(ROUTE)
					.routeId(transferFilesProperties.getProcessName())
					.action(STOP)
	...
	private EndpointConsumerBuilder getSftpOriginServer(TransferFilesProperties properties) {

		return sftp(properties.getOriginServerPath())
				.username(properties.getOriginServerUsername())
				.password(properties.getOriginServerPassword())
				.fileName("ScoreRC_130224.txt")
				.useUserKnownHostsFile(false)
				.sendEmptyMessageWhenIdle(true)
				.binary(true)
				.repeatCount(properties.getRepeatCount())
				.readLock(CHANGED)
				.readLockCheckInterval(5000)
				.readLockMinAge(6000)
				.scheduler(SPRING)
				.schedulerProperties(CRON, properties.getCron())
				.advanced()
				.bridgeErrorHandler(true)
				.throwExceptionOnConnectFailed(true)
				.reconnectDelay(properties.getReconnectDelay())
				.maximumReconnectAttempts(properties.getMaximumReconnectAttempts());
	}

	private EndpointProducerBuilder toScpDestinationServer(TransferFilesProperties properties) {
		return scp(properties.getDestinationServerPath())
				.username(properties.getDestinationServerUsername())
				.password(properties.getDestinationServerPassword());
	}
